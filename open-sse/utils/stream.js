      ...
      for (const line of lines) {
        ...
        if (translated?.length > 0) {
          for (const item of translated) {
            ...
            // If translated item already has valid usage, keep it and apply format filtering if needed
            if (hasValidUsage(item.usage)) {
              item.usage = filterUsageForFormat(item.usage, sourceFormat);
            }
